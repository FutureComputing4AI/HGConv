...
def warmup_exponential_decay_scheduler(lr, steps, warmup, epochs, rate):
    decay_steps = (epochs - warmup) * steps
    return optax.warmup_exponential_decay_schedule(init_value=0.0, peak_value=lr, warmup_steps=warmup * steps,
                                                   transition_steps=steps, decay_rate=rate,
                                                   transition_begin=warmup * steps)
...
if __name__ == '__main__':
    ...
    def scheduler(step_number, steps):
        decayed_value = 0.01 * 0.9 ** (step_number / steps)
        return decayed_value

    steps = 50_000 // 50
    epochs = 200
    # schedule = warmup_exponential_decay_scheduler(lr=0.01, steps=steps, warmup=1, epochs=epochs, rate=0.9)
