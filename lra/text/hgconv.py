...
class Network(nn.Module):
    ...
    def __call__(self, en_in, training: bool = False):
        ...
        for i in range(self.n_layer):
            ...
            cf = self.param(f'cf_{i}', jax.random.normal, (1, 7, self.embed_size))
...
def predict(state, batch, rngs):
    true = batch[1]
    pred = state.apply_fn({'params': state.params, 'batch_stats': state.batch_stats}, batch[0],
                          rngs=rngs, training=False, mutable=False)
    acc = evaluate(true=true, pred=pred)
    metrics = {'acc': acc}
    return metrics
...
def train(batch_size: int, vocab_size: int, max_seq_len: int, embed_size: int,
    ...
    for epoch in range(1, epochs + 1):
        ...
        for x_true, y_true in tqdm(train_loader):
...
if __name__ == '__main__':
    ...
    train(batch_size=5, vocab_size=257, max_seq_len=4096, embed_size=512, n_layer=2,
          output_size=2, dropout_rate=0.0, lr=0.01, epochs=32)
