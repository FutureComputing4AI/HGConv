...
class Block(nn.Module):
    ...
    def __call__(self, en_in, training: bool = False):
        ...
        mask = np.where(x > 0, 1., 0.)[:, :, np.newaxis]

        x = nn.Embed(self.vocab_size, self.embed_size)(x) * mask
        x = nn.Dropout(self.dropout_rate, deterministic=not training)(x)  # (B, T, H)

        for i in range(self.n_layer):
            ef = self.param(f'ef_{i}', jax.random.normal, (self.embed_size,))
            cf = self.param(f'cf_{i}', jax.random.normal, (1, 4, self.embed_size))
        ...
        x = np.sum(x * mask, axis=1) / np.sum(mask, axis=1)
        return x
...
class Network(nn.Module):
    ...
    @nn.compact
    def __call__(self, inputs1, inputs2, training: bool = False):
        inputs1 = inputs1.astype('int32')  # (B, T)
        inputs2 = inputs2.astype('int32')  # (B, T)

        encoder = Block(vocab_size=self.vocab_size,
        ...
        x = np.concatenate([x1, x2, x1 * x2, x1 - x2], axis=-1)
...
def initialize_model(model, max_seq_len, init_rngs):
    ...
    variables = model.init(init_rngs, *(init_inputs, init_inputs))
...
def train_step(state, batch, rngs):
    ...
    def loss_fn(params):
        pred_ = state.apply_fn({'params': params}, *(batch[0], batch[1]), rngs=rngs, training=True)
...
def predict(state, batch, rngs):
    true = batch[2]
    pred = state.apply_fn({'params': state.params}, *(batch[0], batch[1]), rngs=rngs, training=False)
    acc = evaluate(true=true, pred=pred)
    metrics = {'acc': acc}
    return metrics
...
def train(batch_size: int, vocab_size: int, max_seq_len: int, embed_size: int,
    ...
    name = 'retrieval'
    print('exp batch size:', batch_size, 'layers:', n_layer)

    # load dataset
    print('Loading data & building network ...')
    train_loader, test_loader = load_dataset(batch_size=batch_size, max_seq_len=max_seq_len)
    ...
    for epoch in range(1, epochs + 1):
        ...
        for x_true1, x_true2, y_true in tqdm(train_loader):
        ...
        for x_true1, x_true2, y_true in test_loader:
            p_key_next, p_key = jax.random.split(p_key_next)
            d_key_next, d_key = jax.random.split(d_key_next)
            rngs = {'params': shard_prng_key(p_key), 'dropout': shard_prng_key(d_key)}
...
if __name__ == '__main__':
    ...
    train(batch_size=8, vocab_size=128, max_seq_len=4000, embed_size=128, n_layer=6,
          output_size=2, dropout_rate=0.0, lr=0.01, epochs=30)
