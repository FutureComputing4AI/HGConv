...
class Embedding(nn.Module):
    ...
    def __call__(self, inputs):
        ...
        position_embedding = nn.Embed(self.max_seq_len, self.embed_size)(self.positions[:, 0:inputs.shape[1]])
...
class Network(nn.Module):
    ...
    def __call__(self, encoder_input, training: bool = False):
        ...
        # embedding
        x = nn.Embed(self.vocab_size, self.embed_size)(encoder_input)
...
def train_step(state, batch, rngs):
    ...
    def loss_fn(params):
        ...
        loss_ = cross_entropy_loss(y_true=y_true, y_pred=pred_)
        return loss_, pred_
