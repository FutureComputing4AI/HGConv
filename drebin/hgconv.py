...
def train(batch_size, max_seq_len, embed_size=256, lr=1e-2, epochs=10):
    ...
    print(name, 'batch size:', batch_size, 'max seq len:', max_seq_len)

    # load dataset
    train_loader, test_loader = load_dataset(batch_size=batch_size,
                                             max_seq_len=max_seq_len,
                                             file_type="apks/",  # apks or tars
                                             num_workers=10,
                                             drop_last=True)
...
if __name__ == '__main__':
    ...
    test_accuracies = []
    for i in range(10):
        test_accuracies.append(train(batch_size=2, max_seq_len=4096, epochs=100))
